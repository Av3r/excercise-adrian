...
import object_programming.lab3.florist_simple.flowers.*;
...
public class FloristsTest {
    ...
    static int valueOf(Box box, String color) {
        ...
        List<Flower> filteredFlowers = box.getFlowers().stream()
                .filter(flower -> color.equals(flower.getColor()))
                .collect(Collectors.toList());
        for (Flower filteredFlower : filteredFlowers) {
            sum += filteredFlower.getCount() * priceList.getPrice(filteredFlower.getName());
        }
        return sum;
    }
